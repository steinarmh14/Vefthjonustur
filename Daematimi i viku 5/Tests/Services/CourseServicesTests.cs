...
namespace CoursesAPI.Tests.Services
{
	public class CourseServicesTests
	{
		...
		private const string SSN_STEINAR = "0306933379";
		private const string INVALID_SSN = "9876543210";

		private const string NAME_GUNNA  = "Guðrún Guðmundsdóttir";
		private const string NAME_STEINAR = "Steinar Marinó Hilmarsson";

		private const int COURSEID_VEFF_20151 = 1;
		private const int COURSEID_GSKI_20151 = 2;
		...
		public CourseServicesTests()
		{
			...
			var courseTemplates = new List<CourseTemplate>
			{
				...
				{
					CourseID    = "T-213-VEFF",
					Description = "Í þessum áfanga verður fjallað um vefforritun ....",
					Name        = "Vefforitun 2"
				},
				new CourseTemplate
				{
					CourseID    = "T-201-GSKI",
					Description = "Í þessum áfanga verður fjallað um gagnaskipan....",
					Name        = "Gagnaskipan"
				}
			...
			var courses = new List<CourseInstance>
			{
				...
				new CourseInstance
				{
					ID         = COURSEID_VEFF_20151,
					CourseID   = "T-213-VEFF",
					SemesterID = "20151"
				},
				new CourseInstance
				{
					ID         = COURSEID_GSKI_20151,
					CourseID   = "T-201-GSKI",
					SemesterID = "20151"
				}
			};
		...
		public void GetCoursesBySemester_ReturnsEmptyListWhenNoDataDefined()
		{
			...
			// Act:
			var courseList = _service.GetCourseInstancesBySemester();

			Assert.Equal(courseList.Count, 0);
		
		}

		[Fact]
		public void GetCoursesBySemester_ReturnsListOfCoursesOnASemesterWithMoreThanOneCourse()
		{
			// Arrange:
			var semester = "20151";

			// Act:
			var courseList = _service.GetCourseInstancesBySemester(semester);

			//Assert:
			
			//Check size of courseList, it should be equal to 2
			Assert.Equal(courseList.Count(), 2);

			//Check if the right data is in the courseList
			Assert.Equal(courseList[0].CourseInstanceID, COURSEID_VEFF_20151);
			Assert.Equal(courseList[1].CourseInstanceID, COURSEID_GSKI_20151);

		}

		[Fact]
		public void GetCoursesBySemester_ReturnsListOfCoursesOnASemesterWithOneCourse()
		{
			// Arrange:
			var semester = "20153";
			...
			//Check size of courseList, it should be equal to 1
			Assert.Equal(courseList.Count(), 1);

			//Check if the right data is in courseList
			Assert.Equal(courseList[0].CourseInstanceID, COURSEID_VEFT_20153);

		}

		[Fact]
		public void GetCoursesBySemester_ReturnsListOfCourseOnCurrentSemester()
		{
			// Arrange:
		...
		[Fact]
		public void GetCoursesBySemester_ReturnsTheTeacherOfACourse()
		{
			// Arrange:
			var teacher = "Daníel B. Sigurgeirsson";
			...
			//Check if the right data is in courseList
			Assert.Equal(courseList[0].MainTeacher, teacher);

		}

		public void GetCoursesBySemester_ReturnsEmptyStringIfNoMainTeacher()
		{
			// Arrange:
			var semester = "20163";
			// Act:
			...
			//Check if the right data is in courseList
			Assert.Equal(courseList[0].MainTeacher, "");
